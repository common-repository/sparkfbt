...
use Sparkfbt\SparkPlugins\SparkWoo\Common\Options\BooleanOption;
use Sparkfbt\SparkPlugins\SparkWoo\Common\Plugins\GlobalVariables;
use Sparkfbt\SparkPlugins\SparkWoo\Common\StylesScripts\StylesScriptsModule;
        ...
        <div class="grow relative w-full">
          <?php 
                if (\count($products) === 0) {
                    ?>
            <p><?php 
                    esc_html_e('Unfortunately, there are no products to show here...', 'sparkfbt');
                    ?></p>
          <?php 
                } else {
                    ?>
            <?php 
                    if (!$useThemeNumberOfColumns && $useThemeColumnsSetting) {
                        ...
                        // Set columns WM
                        if (\function_exists('Sparkfbt\\woodmart_loop_prop') && \function_exists('Sparkfbt\\woodmart_set_loop_prop')) {
                    ...
                    if (!$useThemeNumberOfColumns && $useThemeColumnsSetting) {
                        ...
                        // Restore columns WM
                        if (\function_exists('Sparkfbt\\woodmart_set_loop_prop')) {
        ...
        if ('' != $titleAboveProducts && $titleAboveProducts) {
          ...
          <<?php 
            ...
            if ($this->isMultiLanguageOption->getValue()) {
                ?>
              <?php 
                esc_html_e('We recommend these products for you!', 'sparkfbt');
                ?>
            <?php 
            } else {
                ?>
