...
namespace Sparkfbt\MathPHP\InformationTheory;
...
class Entropy
{
    ...
    public static function shannonEntropy(array $p) : float
    {
        ...
        // ∑ pᵢlog₂(pᵢ)
        $∑pᵢlog₂⟮pᵢ⟯ = \array_sum(\array_map(function ($pᵢ) {
            return $pᵢ * \log($pᵢ, 2);
        }, $p));
        return -$∑pᵢlog₂⟮pᵢ⟯;
    }
    ...
    public static function shannonNatEntropy(array $p)
    {
        ...
        // ∑ pᵢln(pᵢ)
        $∑pᵢln⟮pᵢ⟯ = \array_sum(\array_map(function ($pᵢ) {
            return $pᵢ * \log($pᵢ);
        }, $p));
        return -$∑pᵢln⟮pᵢ⟯;
    }
    ...
    public static function crossEntropy(array $p, array $q)
    {
        ...
        // ∑ p(x) log₂ q(x)
        $∑plog₂⟮q⟯ = \array_sum(\array_map(function ($pᵢ, $qᵢ) {
            return $pᵢ * \log($qᵢ, 2);
        }, $p, $q));
        return -$∑plog₂⟮q⟯;
    }
