...
namespace Sparkfbt\MathPHP\LinearAlgebra;

use Sparkfbt\MathPHP\Functions\Map;
use Sparkfbt\MathPHP\Functions\Support;
use Sparkfbt\MathPHP\Exception;
use Sparkfbt\MathPHP\LinearAlgebra\Reduction;
/**
 * m x n Matrix
 *
...
class NumericMatrix extends Matrix
{
    ...
    protected const ε = 9.999999999999999E-12;
    ...
    public function isRef() : bool
    {
        ...
        // All nonzero rows are above any rows of all zeroes
        for ($i = $m - 1; $i >= 0; $i--) {
            $zero_row = \count(\array_filter($this->A[$i], function ($x) {
                return $x != 0;
            })) === 0;
            if (!$zero_row) {
                $zero_row_ok = \false;
    ...
    public function kroneckerProduct(NumericMatrix $B) : NumericMatrix
    {
        ...
        foreach ($arrays as $row) {
            ...
            $matrices[] = \array_reduce($row, function (NumericMatrix $augmented_matrix, NumericMatrix $matrix) {
        ...
        $A⊗B = \array_reduce($matrices, function (NumericMatrix $augmented_matrix, NumericMatrix $matrix) {
    ...
    public function inverse() : NumericMatrix
    {
        ...
        if ($m === 2) {
            ...
            $c = $A[1][0];
            $d = $A[1][1];
            $R = MatrixFactory::createNumeric([[$d, -$b], [-$c, $a]], $this->ε);
    ...
    public function meanDeviation(string $direction = 'rows') : NumericMatrix
    {
        ...
        return $direction === self::ROWS ? $this->meanDeviationOfRowVariables() : $this->meanDeviationOfColumnVariables();
    }
    /**
     * Mean deviation matrix
    ...
    public function meanDeviationOfRowVariables() : NumericMatrix
    {
        ...
        $M = $this->rowMeans();
        /** @var Vector[] $B */
        $B = \array_map(function (Vector $Xᵢ) use($M) {
    ...
    public function covarianceMatrix(string $direction = 'rows') : NumericMatrix
    {
        ...
        $S = $direction === self::ROWS ? $this->covarianceMatrixOfRowVariables() : $this->covarianceMatrixOfColumnVariables();
        return $S;
    }
    /**
     * Covariance matrix (variance-covariance matrix, sample covariance matrix)
    ...
    {
        $sums = \array_map(function (array $row) {
    ...
    {
        $n = $this->n;
        $means = \array_map(function (array $row) use($n) {
    ...
    public function __toString() : string
    {
        ...
        return \trim(\array_reduce(\array_map(function ($mᵢ) {
            return '[' . \implode(', ', $mᵢ) . ']';
        }, $this->A), function ($A, $mᵢ) {
            return $A . \PHP_EOL . $mᵢ;
        }));
    }
    /**
    ...
    {
        return ['matrix' => \sprintf('%dx%d', $this->m, $this->n), 'data' => \PHP_EOL . (string) $this, 'ε' => $this->ε];
    }
}
