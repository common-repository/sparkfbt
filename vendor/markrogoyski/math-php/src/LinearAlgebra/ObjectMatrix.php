...
use Sparkfbt\MathPHP\Number\ArbitraryInteger;
    ...
    public function multiply($B) : Matrix
    {
        ...
        for ($i = 0; $i < $m; $i++) {
            for ($j = 0; $j < $n; $j++) {
                ...
                $VB = $B->getColumn($j);
                $R[$i][$j] = \array_reduce(\array_map(function (ObjectArithmetic $a, ObjectArithmetic $b) {
                    return $a->multiply($b);
                }, $VA, $VB), function ($sum, $item) {
                    return $sum ? $sum->add($item) : $item;
                });
            }
        }
        /** @var ObjectMatrix */
