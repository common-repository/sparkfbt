...
class Quaternion implements ObjectArithmetic
{
    ...
    public function add($q) : Quaternion
    {
        if (!\is_numeric($q) && !$q instanceof Quaternion) {
            throw new Exception\IncorrectTypeException('Argument must be real or quaternion' . \print_r($q, \true));
        }
        if (\is_numeric($q)) {
            $r = $this->r + $q;
    ...
    public function multiply($q) : Quaternion
    {
        ...
        return new Quaternion($a₁ * $a₂ - $b₁ * $b₂ - $c₁ * $c₂ - $d₁ * $d₂, $b₁ * $a₂ + $a₁ * $b₂ + $c₁ * $d₂ - $d₁ * $c₂, $a₁ * $c₂ - $b₁ * $d₂ + $c₁ * $a₂ + $d₁ * $b₂, $a₁ * $d₂ + $b₁ * $c₂ - $c₁ * $b₂ + $d₁ * $a₂);
    }
    /**
     * Quaternion division
    ...
    {
        return \abs($this->r - $q->r) < self::EPSILON && \abs($this->i - $q->i) < self::EPSILON && \abs($this->j - $q->j) < self::EPSILON && \abs($this->k - $q->k) < self::EPSILON;
    }
    /**************************************************************************
     * PRIVATE FUNCTIONS
     **************************************************************************/
