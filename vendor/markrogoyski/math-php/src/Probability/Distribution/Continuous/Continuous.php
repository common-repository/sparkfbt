...
namespace Sparkfbt\MathPHP\Probability\Distribution\Continuous;

abstract class Continuous extends \Sparkfbt\MathPHP\Probability\Distribution\Distribution implements ContinuousDistribution
{
    ...
    public function inverse(float $target)
    {
        ...
        while ($dif > $tolerance) {
            ...
            $guess_history["{$guess}"] = isset($guess_history["{$guess}"]) ? $guess_history["{$guess}"] + 1 : 0;
            if ($guess_history["{$guess}"] > self::GUESS_THRESHOLD) {
                $repeated_guesses = \array_filter($guess_history, function ($repeated_guess) {
     ...
     * @return int|float
     */
    public abstract function median();
}
