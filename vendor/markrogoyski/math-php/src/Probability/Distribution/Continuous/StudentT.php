...
use Sparkfbt\MathPHP\Functions\Special;
...
class StudentT extends Continuous
{
     ...
     * @var array{"ν": string}
     */
    public const PARAMETER_LIMITS = ['ν' => '(0,∞)'];
    /**
     * Distribution support bounds limits
     * t ∈ (-∞,∞)
     * @var array{"t": string}
     */
    public const SUPPORT_LIMITS = ['t' => '(-∞,∞)'];
    ...
    public function cdf(float $t) : float
    {
        ...
        if ($nx > 1.0E+100) {
            ...
            $beta1 = new Beta(0.5, $ν / 2);
            $beta2 = new Beta($ν / 2, 0.5);
            $val = $ν > $t * $t ? 0.5 - $beta1->cdf($t * $t / ($ν + $t * $t)) + 0.5 : $beta2->cdf(1 / $nx);
        }
        $lowerTail = $t > 0;
        $val /= 2;
        return $lowerTail ? 0.5 - $val + 0.5 : $val;
        // 1 - p
    }
    /**
     * Inverse 2 tails
