...
namespace Sparkfbt\MathPHP\Sequence;

use Sparkfbt\MathPHP\Exception\OutOfBoundsException;
use Sparkfbt\MathPHP\NumberTheory\Integer;
/**
...
class Advanced
{
    ...
    private const PERFECT_NUMBERS = [6, 28, 496, 8128, 33550336, 8589869056, 137438691328, 2305843008139952128, 2.658455991569832E+36, 1.915619426082361E+53];
    /**
     * Perfect numbers
    ...
    public static function primesUpTo(int $n) : array
    {
        ...
        $primes = \array_fill_keys(\range(2, $n), \true);
        $√n = \ceil(\sqrt($n));
        for ($i = 2; $i <= $√n; $i++) {
            if ($primes[$i] === \true) {
                $i² = $i ** 2;
