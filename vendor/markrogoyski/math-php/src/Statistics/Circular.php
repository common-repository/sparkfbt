...
class Circular
{
    ...
    {
        $∑sinαⱼ = \array_sum(\array_map(function ($αⱼ) {
            return \sin($αⱼ);
        }, $angles));
        $∑cosαⱼ = \array_sum(\array_map(function ($αⱼ) {
            return \cos($αⱼ);
        }, $angles));
    ...
    {
        $S = \array_sum(\array_map(function ($θᵢ) {
            return \sin($θᵢ);
        }, $angles));
        $C = \array_sum(\array_map(function ($θᵢ) {
            return \cos($θᵢ);
        }, $angles));
        $S² = $S ** 2;
        $C² = $C ** 2;
    ...
    {
        return ['n' => \count($angles), 'mean' => self::mean($angles), 'resultant_length' => self::resultantLength($angles), 'mean_resultant_length' => self::meanResultantLength($angles), 'variance' => self::variance($angles), 'sd' => self::standardDeviation($angles)];
    }
}
