...
namespace Sparkfbt\MathPHP\Statistics;

use Sparkfbt\MathPHP\Exception;
use Sparkfbt\MathPHP\Functions\Map;
use Sparkfbt\MathPHP\LinearAlgebra\Eigenvalue;
use Sparkfbt\MathPHP\LinearAlgebra\Eigenvector;
use Sparkfbt\MathPHP\LinearAlgebra\NumericMatrix;
use Sparkfbt\MathPHP\LinearAlgebra\MatrixFactory;
use Sparkfbt\MathPHP\Probability\Distribution\Continuous\ChiSquared;
use Sparkfbt\MathPHP\Probability\Distribution\Continuous\StandardNormal;
use Sparkfbt\MathPHP\Trigonometry;
/**
...
class Correlation
{
    ...
    public static function weightedCovariance(array $X, array $Y, array $w) : float
    {
        ...
        $∑wᵢ⟮xᵢ − μₓ⟯⟮yᵢ − μy⟯ = \array_sum(\array_map(function ($xᵢ, $yᵢ, $wᵢ) use($μₓ, $μy) {
    ...
    {
        return $population ? self::populationCorrelationCoefficient($X, $Y) : self::sampleCorrelationCoefficient($X, $Y);
    }
    /**
    ...
    public static function populationCorrelationCoefficient(array $X, array $Y) : float
    {
        ...
        $σx = Descriptive::standardDeviation($X, \true);
        $σy = Descriptive::standardDeviation($Y, \true);
    ...
    public static function weightedCorrelationCoefficient(array $X, array $Y, array $w) : float
    {
        ...
        $sxw = Descriptive::weightedSampleVariance($X, $w, \true);
        $syw = Descriptive::weightedSampleVariance($Y, $w, \true);
    ...
    {
        return ['cov' => self::covariance($X, $Y, $population), 'r' => self::r($X, $Y, $population), 'r2' => self::r2($X, $Y, $population), 'tau' => self::kendallsTau($X, $Y), 'rho' => self::spearmansRho($X, $Y)];
    }
    /**
