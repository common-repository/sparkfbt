...
class Descriptive
{
    ...
    public static function weightedSampleVariance(array $numbers, array $weights, bool $biased = \false) : float
    {
        ...
        $∑wᵢ⟮xᵢ − μw⟯² = \array_sum(\array_map(function ($xᵢ, $wᵢ) use($μw) {
            return $wᵢ * \pow($xᵢ - $μw, 2);
        }, $numbers, $weights));
        $∑wᵢ = $biased ? \array_sum($weights) : \array_sum($weights) - 1;
        return $∑wᵢ⟮xᵢ − μw⟯² / $∑wᵢ;
    }
    /**
     * Standard deviation
    ...
    public static function medianAbsoluteDeviation(array $numbers) : float
    {
        ...
        return Average::median(\array_map(function ($xᵢ) use($x) {
            return \abs($xᵢ - $x);
        }, $numbers));
    }
    /**
    ...
    public static function quartilesExclusive(array $numbers) : array
    {
        ...
        if (\count($numbers) === 1) {
            ...
            return ['0%' => $number, 'Q1' => $number, 'Q2' => $number, 'Q3' => $number, '100%' => $number, 'IQR' => 0];
        }
        \sort($numbers);
