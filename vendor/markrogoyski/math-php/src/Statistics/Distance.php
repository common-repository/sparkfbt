...
class Distance
{
    ...
    public static function hellinger(array $p, array $q) : float
    {
        ...
        $p = \array_map(function ($pᵢ) {
            return $pᵢ == 0 ? 1.0E-15 : $pᵢ;
        }, $p);
        $q = \array_map(function ($qᵢ) {
            return $qᵢ == 0 ? 1.0E-15 : $qᵢ;
        }, $q);
        // √ ∑ (√pᵢ - √qᵢ)²
        $√∑⟮√pᵢ − √qᵢ⟯² = \sqrt(\array_sum(\array_map(function ($pᵢ, $qᵢ) {
    ...
    public static function minkowski(array $xs, array $ys, int $p) : float
    {
        ...
        $∑｜xᵢ − yᵢ⟯ᵖ = \array_sum(\array_map(function ($x, $y) use($p) {
            return \abs($x - $y) ** $p;
        }, $xs, $ys));
        return $∑｜xᵢ − yᵢ⟯ᵖ ** (1 / $p);
    }
    ...
    public static function brayCurtis(array $u, array $v) : float
    {
        ...
        $∑｜uᵢ − vᵢ｜ = \array_sum(\array_map(function (float $uᵢ, float $vᵢ) {
            return \abs($uᵢ - $vᵢ);
        }, $u, $v));
        $∑｜uᵢ ＋ vᵢ｜ = \array_sum(\array_map(function (float $uᵢ, float $vᵢ) {
            return \abs($uᵢ + $vᵢ);
    ...
    public static function canberra(array $p, array $q) : float
    {
        ...
        // Numerators ｜pᵢ − qᵢ｜
        $｜p − q｜ = \array_map(function (float $pᵢ, float $qᵢ) {
            return \abs($pᵢ - $qᵢ);
        }, $p, $q);
        // Denominators ｜pᵢ｜ + ｜qᵢ｜
        $｜p｜ ＋ ｜q｜ = \array_map(function (float $p, float $q) {
            return \abs($p) + \abs($q);
        }, $p, $q);
        ...
        // ∑ --------------
        //   ｜pᵢ｜ + ｜qᵢ｜
        return \array_sum(\array_map(function (float $｜pᵢ − qᵢ｜, float $｜pᵢ｜ ＋ ｜qᵢ｜) {
            return $｜pᵢ｜ ＋ ｜qᵢ｜ == 0 ? 0 : $｜pᵢ − qᵢ｜ / $｜pᵢ｜ ＋ ｜qᵢ｜;
        }, $｜p − q｜, $｜p｜ ＋ ｜q｜));
    }
    /**
     * Chebyshev Distance
    ...
    public static function chebyshev(array $xs, array $ys) : float
    {
        ...
        return (float) \max(\array_map(function (float $xᵢ, $yᵢ) {
            return \abs($xᵢ - $yᵢ);
        }, $xs, $ys));
    }
}
