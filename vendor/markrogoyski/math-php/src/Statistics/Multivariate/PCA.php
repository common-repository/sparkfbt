...
namespace Sparkfbt\MathPHP\Statistics\Multivariate;

use Sparkfbt\MathPHP\Exception;
use Sparkfbt\MathPHP\Functions\Map\Single;
...
use Sparkfbt\MathPHP\LinearAlgebra\Vector;
use Sparkfbt\MathPHP\Probability\Distribution\Continuous\F;
use Sparkfbt\MathPHP\Probability\Distribution\Continuous\StandardNormal;
use Sparkfbt\MathPHP\Statistics\Descriptive;
/**
...
class PCA
{
     ...
     */
    public function __construct(NumericMatrix $M, bool $center = \true, bool $scale = \true)
    {
        ...
        $this->center = $center === \true ? $this->center = $M->columnMeans() : ($this->center = new Vector(\array_fill(0, $M->getN(), 0)));
        if ($scale === \true) {
            $scaleArray = [];
            for ($i = 0; $i < $M->getN(); $i++) {
    ...
    public function getQResiduals(NumericMatrix $new_data = null) : NumericMatrix
    {
        ...
        $P = $this->EVec->submatrix(0, 0, $vars - 1, 0);
        // Get the first column of the loading matrix
        $P′ = $P->transpose();
        /** @var NumericMatrix $Q */
     ...
     */
    public function getCriticalT2(float $alpha = 0.05) : array
    {
        $samp = $this->data->getM();
        $vars = $this->data->getN();
        $T²Critical = [];
     ...
     * @throws Exception\MathException
     */
    public function getCriticalQ(float $alpha = 0.05) : array
    {
        $vars = $this->data->getN();
        $QCritical = [];
        for ($i = 0; $i < $vars - 1; $i++) {
